...
class ProcessDeposit : CommonProcessor() {

    override fun cleanUpHtmlSpecific(text: String): String = text
    ...
    override fun transformToCsv(row: String) =
        row.replace(
...
//            "$1\t$3\t$5\t\t\t\t$4\t$2:00\t\t\t$6\t=ОКРУГЛ(R[-1]C[-1]+RC[-9];2)\t$3"
            ...
            "$1\t$3\t$4\t\t\t\t$2\t00:00\t\t\t$5\t=ОКРУГЛ(R[-1]C[-1]+RC[-9];2)\t$3"
        )
}